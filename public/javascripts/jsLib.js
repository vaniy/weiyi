...
Mc.Global.mc_isAjax = [];
Mc.Config = {};
//Mc.Config.AjaxUrl = "http://121.40.186.49:8120";
//Mc.Config.AjaxUrl = "http://localhost:4995";
//Mc.Config.AjaxUrl = "http://api.zjzramc.cn";
Mc.Config.AjaxUrl = "";
//Mc.Config.AjaxUrl = "http://localhost:10012";
Mc.Ajax = function (options) {
    ...
    function ajax() {
        //todo 要删除
        settings.url = settings.url.replace('Api/', '').replace('api/', '');
        $.ajax({
    ...
    var ms = {
        ...
        init: function (obj, args) {
            return (function () {
                ...
                //fix
                ms.msData = { isAjax: false, pageCount: Math.ceil(args.allCount / args.pageSize), pageIndex: 1 }
                ms.getArgs(obj); //获取参数
                if (args.isLoadFirst) { //如果要自动加载第一页
        ...
        fillHtml: function (obj, args) {
            return (function () {
                ...
                //上一页
                if (args.current > 1) {
                    obj.append('<div class="edge-item fp prevPage"><i class="ic-page-num-left"></i></div>');
                } else {
                    obj.remove('.prevPage');
                    obj.append('<div class="edge-item fp disable"><i class="ic-page-num-left"></i></div>');
                }
                //中间页码
                if (args.current != 1 && args.current >= 4 && args.pageCount != 4) {
                    obj.append('<div class="num-item item fp tcdNumber"><span>' + 1 + '</span></div>');
                }
                if (args.current - 2 > 2 && args.current <= args.pageCount && args.pageCount > 5) {
                    obj.append('<div class="num-item item fp ellipsis"><span>...</span></div>');
                }
                ...
                for (; start <= end; start++) {
                    if (start <= args.pageCount && start >= 1) {
                        if (start != args.current) {
                            obj.append('<div class="num-item item fp tcdNumber"><span>' + start + '</span></div>');
                        } else {
                            obj.append('<div class="num-item item fp active"><span>' + start + '</span></div>');
                        }
                    }
                }
                ...
                if (args.current != args.pageCount && args.current < args.pageCount - 2 && args.pageCount != 4) {
                    obj.append('<div class="num-item item fp tcdNumber"><span>' + args.pageCount + '</span></div>');
                }
                //下一页
                if (args.current < args.pageCount) {
                    obj.append('<div class="edge-item fp nextPage"><i class="ic-page-num-right"></i></div>');
                } else {
                    obj.remove('.nextPage');
                    obj.append('<div class="edge-item fp disable"><i class="ic-page-num-right"></i></div>');
                }
        ...
        ajaxNewData: function (obj, args) {
            $.ajax({
                ...
                async: args.async,
                success: function (result) {
                    ms.msData.pageCount = Math.ceil(args.allCount / args.pageSize);; //获得总页数
                    args.success.call(obj, result); //success回调
                },
                beforeSend: function () {
...
Mc.App.Pop.Tip = function (options) {
    var settings = $.extend({
        ...
        tipLive: 800 //提示层生存时间(毫秒) | 默认 2000
    }, options);
